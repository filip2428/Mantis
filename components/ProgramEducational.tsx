...
export default function ProgramCard({
  ...
  const contactHref = useMemo(
    () => ({ pathname: "/contact", query: { program: title } }),
    [title]
  );
        ...
        <div className="flex flex-grow flex-col p-6">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            <Link
              ...
              className="inline-flex items-center justify-center rounded-full border border-mantis-green-200 bg-white px-4 py-1.5 text-sm font-semibold text-mantis-green-700 shadow-sm transition hover:border-mantis-green-300 hover:bg-mantis-cream/60"
            >
              Rezervă un loc
            </Link>
          </div>
          ...
          <>
            ...
            >
              <div className="relative flex-shrink-0">
                ...
                <button
                  onClick={() => setIsOpen(false)}
                  className="absolute right-4 top-4 rounded-full bg-white/90 p-2 text-mantis-bark shadow-sm transition-colors hover:bg-white dark:bg-[#102a1b]/80 dark:text-white"
                >
                  <X className="h-6 w-6" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-8">
                ...
                <div className="mt-10 flex flex-col gap-4 rounded-2xl border border-mantis-green-100/80 bg-white/70 p-6 text-mantis-bark dark:border-[#1b3525] dark:bg-[#102a1b]/70 dark:text-white">
                  <div>
                    <h3 className="text-lg font-semibold text-mantis-green-700 dark:text-mantis-green-300">
                      Gata să rezervi?
                    </h3>
                    <p className="mt-2 text-sm text-mantis-bark/80 dark:text-gray-300">
                      Completează formularul nostru de contact și menționăm automat acest program în mesajul tău pentru un răspuns rapid.
                    </p>
                  </div>
                  <Link
                    href={contactHref}
                    className="inline-flex items-center justify-center rounded-full bg-mantis-green-600 px-6 py-3 text-sm font-semibold text-white shadow-mantis-soft transition hover:bg-mantis-green-700"
                  >
